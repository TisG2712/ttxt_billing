...
public interface UserRepository extends JpaRepository<Partner, Long> {
        ...
        Partner getPasswordPartner(Long id);
        ...
        @Query("select new com.example.be_quan_tri.dtos.partners.CustomReportDto(pts.partner.id as idPartner, pts.partner.partnerName as partnerName, pts.partner.partnerCode as partnerCode, sum (r.totalRequestSuccess + r.totalRequestFailure) as totalRequests , sum (CASE WHEN  r.createdAt >= g.dateGolive THEN r.totalRequestSuccess ELSE 0 END) as totalSuccessfulRequests, SUM(CASE WHEN  r.createdAt >= g.dateGolive THEN pts.serviceChildren.servicePrice * r.totalRequestSuccess ELSE 0 END) as totalCost) from "
                        ...
                        +
                        " where EXTRACT(YEAR FROM r.createdAt) = :nowYear and (g.dateGolive is null or r.createdAt >= g.dateGolive) and (pts.partner.id = :partnerId or :partnerId is null ) and pts.service.status = 1 and r.createdAt between :startDate and :endDate group by pts.partner.id, pts.partner.partnerName, pts.partner.partnerCode")
        Page<CustomReportDto> findByUpdatedAt(Long partnerId, Integer nowYear, LocalDateTime startDate, LocalDateTime endDate,
                        Pageable pageable);
        ...
        Page<CustomReportDto> findByUpdatedAt2(Long partnerId, LocalDateTime startDate, LocalDateTime endDate,
                                              Pageable pageable);
        ...
        @Query("select new com.example.be_quan_tri.dtos.partners.CustomListPartnerDtos(pts.partner.id as partnerId, pts.partner.partnerName as partnerName, " +
                " pts.partner.partnerCode as partnerCode , pts.partner.partnerUser as partnerUser, pts.partner.createdAt as createdAt, " +
                " pts.status.id as idStatus, pts.status.statusDetail ) from " +
                " Partner_to_Status pts where pts.status.id = :id")
        Page<CustomListPartnerDtos> findByUpdatedAt(Long id, Pageable pageable);

        @Query("select new com.example.be_quan_tri.dtos.partners.CustomListPartnerGroupDtos(ptg.partner.id as partnerId, ptg.partner.partnerName as partnerName, " +
                " ptg.partner.partnerCode as partnerCode , ptg.partner.partnerUser as partnerUser, ptg.partner.createdAt as createdAt, " +
                " ptg.groupPartnerEntity.id as idGroup, ptg.groupPartnerEntity.nameGroup as groupName ) from " +
                " Partner_to_group ptg where ptg.groupPartnerEntity.id = :id")
        Page<CustomListPartnerGroupDtos> findCustomPartnerGroup(Long id, Pageable pageable);

        @Query("select new com.example.be_quan_tri.dtos.partners.CustomPartnerLiteDtos(p.id , p.partnerName, p.partnerCode, p.partnerDetail, p.partnerUser) " +
                " from Partner p")
        List<CustomPartnerLiteDtos> customGetPartnerLite();

        @Query("select new com.example.be_quan_tri.dtos.reports.ReportGroupPartner(ptg.groupPartnerEntity.id as id, ptg.groupPartnerEntity.nameGroup as groupName, " +
                " sum(case when r.createdAt >= g.dateGolive then r.totalRequestSuccess + r.totalRequestFailure end) as totalRequest, " +
                " sum(case when r.createdAt >= g.dateGolive then r.totalRequestSuccess end) as totalSuccess, " +
                " sum(case when r.createdAt >= g.dateGolive then r.totalRequestSuccess * pts.serviceChildren.servicePrice end) as totalMoney) from" +
                " Partner_to_Service pts left join ReportDayEntity r on r.service.id = pts.service.id and r.partner.id = pts.partner.id " +
                " left join Partner_to_group ptg on ptg.partner.id = pts.partner.id" +
                " left join Golive g on g.partner.id = pts.partner.id and g.categoryServices.id = pts.service.id " +
                " where (:idGroup IS NULL OR ptg.groupPartnerEntity.id = :idGroup) and r.createdAt between :start and :end group by ptg.groupPartnerEntity.nameGroup, ptg.groupPartnerEntity.id")
        List<ReportGroupPartner> customGetAllGroup(LocalDateTime start, LocalDateTime end, Long idGroup);

        @Query("select new com.example.be_quan_tri.dtos.reports.ReportPartnerDtosService(ptg.partner.id as id, ptg.partner.partnerCode as partnerCode, " +
                " sum(case when r.createdAt >= g.dateGolive then r.totalRequestSuccess + r.totalRequestFailure else 0 end) as totalTransaction, " +
                " sum(case when r.createdAt >= g.dateGolive then r.totalRequestSuccess else 0 end) as totalTransactionSuccess, " +
                " sum(case when r.createdAt >= g.dateGolive then r.totalRequestSuccess * pts.serviceChildren.servicePrice else 0 end) as totalMoney) from" +
                " Partner_to_Service pts left join ReportDayEntity r on r.service.id = pts.service.id and r.partner.id = pts.partner.id " +
                " left join Partner_to_group ptg on ptg.partner.id = pts.partner.id" +
                " left join Golive g on g.partner.id = pts.partner.id and g.categoryServices.id = pts.service.id " +
                " where (:idGroup IS NULL OR ptg.groupPartnerEntity.id = :idGroup) and r.createdAt between :start and :end group by ptg.partner.id, ptg.partner.partnerCode")
        List<ReportPartnerDtosService> customGetAllPartnerGroup(LocalDateTime start, LocalDateTime end, Long idGroup);
}
