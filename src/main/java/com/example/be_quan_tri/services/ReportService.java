...
public class ReportService {
    ...
    public Page<CustomReportDto> findByUpdatedAt(Long partnerId, Integer nowYear, LocalDateTime startDate, LocalDateTime endDate, Pageable pageable) {

        return userRepository.findByUpdatedAt(partnerId,nowYear, startDate, endDate, pageable);

    }

    public Page<CustomReportDto> findByUpdatedAt2(Long partnerId, LocalDateTime startDate, LocalDateTime endDate, Pageable pageable) {

        return userRepository.findByUpdatedAt2(partnerId, startDate, endDate, pageable);
    ...
    public ResponseEntity<GloableReportResponse> exportDataListPartnerReport(HttpServletResponse httpServletResponse, Long idPartner, LocalDateTime localDateTimeStart, LocalDateTime localDateTimeEnd, Pageable pageable) throws IOException {
        Page<CustomReportDto> page = userRepository.findByUpdatedAt2(idPartner, localDateTimeStart, localDateTimeEnd, pageable);
    ...
    public void exportDataListTransaction(Long partnerId, HttpServletResponse httpServletResponse, String partnerCodeTran, String statusTran, LocalDateTime localDateTimeStart, LocalDateTime localDateTimeEnd) throws IOException {
        List<TransactionExportDtos> transactions = transactionRepository.findByTimeRequestAndApiKey(partnerId, partnerCodeTran, statusTran, localDateTimeStart, localDateTimeEnd);
        excelExportService.exportDataToTransaction(httpServletResponse, transactions);
    ...
    public List<ReportGroupPartner> customGetAllGroup(LocalDateTime start, LocalDateTime end, Long idGroup) {

        return userRepository.customGetAllGroup(start, end, idGroup);

    }
    public List<ReportPartnerDtosService> customGetAllPartnerGroup(LocalDateTime start, LocalDateTime end, Long idGroup) {

        return userRepository.customGetAllPartnerGroup(start, end, idGroup);

    }

    public void exportDataPartnerGroup(HttpServletResponse httpServletResponse, LocalDateTime localDateTimeStart, LocalDateTime localDateTimeEnd, Long idGroup) throws IOException {
        List<ReportGroupPartner> reportGroupPartners = userRepository.customGetAllGroup(localDateTimeStart, localDateTimeEnd, idGroup);
        excelExportService.exportDataPartnerGroup(httpServletResponse, reportGroupPartners);
    }
}
