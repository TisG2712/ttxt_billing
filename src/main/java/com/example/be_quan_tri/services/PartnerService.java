...
//    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<GloableResponse<UserRegisterDto>> registerUser( String data, MultipartFile file) throws IOException {
        ...
        return ResponseEntity.ok(new GloableResponse<>(success, message, null));
    }
    ...
    public ResponseEntity<GloableResponse<ResponsePartner>> updatePartner(String data, MultipartFile file) throws JsonProcessingException {
        ...
        if (file.isEmpty()) {
            success = false;
            message = "Không có file";
            return ResponseEntity.ok(new GloableResponse<>(
                    success, message, null
            ));
        }
        try{
    ...
    public ResponseEntity<GloableResponse<RequestCheckPassword>> checkPassword(RequestCheckPassword checkPassword) {
        ...
        try {
            Partner partner = userRepository.findById(Long.valueOf(checkPassword.getPartnerId())).get();
            System.out.println(partner.getPassword());
            System.out.println(checkPassword.getPassword());

            if (passwordEncoder.matches(checkPassword.getPassword(), partner.getPassword())) {
                partner.setPassword(passwordEncoder.encode(checkPassword.getNewPassword()));
                userRepository.save(partner);
                success = true;
    ...
    public ResponseEntity<GloableResponseList<CustomThongKeAdmin>> thongKe(Long idPartner) {
        ...
        return ResponseEntity.ok(new GloableResponseList<>(
    ...
    public Page<CustomListPartnerDtos> getPartnerStatus(Long id , Pageable pageable) {
        return userRepository.findByUpdatedAt(id, pageable);
    }

    public Page<CustomListPartnerGroupDtos> getPartnerGroup(Long id, Pageable pageable) {
        return userRepository.findCustomPartnerGroup(id, pageable);
    }

    public List<CustomPartnerLiteDtos> customGetPartnerLite() {
        return userRepository.customGetPartnerLite();
    }
}
